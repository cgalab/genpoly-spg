...
public:
	...
	~BSTNode() {delete left; delete right; delete parent;}
	...
	BSTNode* next() {
		// if there is a right child, then it will be the next node, or one of its left grandchildren if they exist
		if (right != NULL) {
			BSTNode* next = right;
			...
			// check if this node is its parents left child, then the parent is the next node.
			if (this == (*parent).left)
				return parent;
			else return NULL;
		}
	...
	std::pair<BSTNode*, enum bst_t> del(BSTNode* val) {
...
public:
	...
	std::pair<BSTNode*, enum bst_t> del(double val) {
		std::pair<BSTNode*, enum bst_t> response;
		response = root->del(val);
		return response;
	}
