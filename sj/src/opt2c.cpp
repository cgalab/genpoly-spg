    ...
    if (bef && af) {
      ...
      if (isval == IS_4P_COLLINEAR) {
        ...
        if (coll4Swap(before, after, edgeS, polygon, points)) {
          ...
          valid = E_SKIP;
          update_lowest_index_c(before, after, lowest_index);
        }
      }
...
//          std::cerr << "4P coll. after swap: " << e << " and bef: " << before << std::endl;
          valid = E_SKIP;
          update_lowest_index_c(e, before, lowest_index);
        }
      }
      else {
...
//          std::cerr << "4P coll. after swap: " << e << " and " << after << std::endl;
          valid = E_SKIP;
          update_lowest_index_c(e, after, lowest_index);
        }
      }
      else {
...
//        std::cerr << std::endl << "removing e1: " << e1 << std::endl;
        val1.first = removeEdgeFromSetc(e1, lowest_index, edgeS, polygon, points);
        if (val1.first == E_SKIP) {
          ...
          if ((revert && (*e2.p1 == *p1)) || (!revert && (*e2.p2 == *p1))) {
            val2.first = removeEdgeFromSetc(e2, lowest_index, edgeS, polygon, points);
...
//        std::cerr << std::endl << "processing e1: " << e1 << std::endl;
        val1 = processEdgec(e1, lowest_index, edgeS, polygon, points);
        if (val1.first == E_SKIP) {
          if(revert)rev_found=true;
          else {edgeS_old = edgeS;rev_found=false;old_index=index;}
          loop=true;revert=true;continue;}
...
//        std::cerr << std::endl << "processing e2: " << e2 << std::endl;
        val2 = processEdgec(e2, lowest_index, edgeS, polygon, points);
        if (val2.first == E_NOT_VALID) break;
        if (val2.first == E_SKIP) {
          val1.first = removeEdgeFromSetc(e1, lowest_index, edgeS, polygon, points);
          if (val1.first == E_NOT_VALID) break;
